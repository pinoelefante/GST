...
public class Interfaccia {
						...
						if(GestioneSerieTV.aggiungiSerie(st)){
							...
							if(!GestioneSerieTV.getSubManager().associaSerie(st)){
								if(Settings.isRicercaSottotitoli()){
									int scelta=(JOptionPane.showConfirmDialog(frame, "Non � stato possibile associare la serie a ItaSA.\nVuoi associarla manualmente?", "Associa ItaSA", JOptionPane.YES_NO_OPTION));
									if(scelta==JOptionPane.YES_OPTION){
										associaFrame();
		...
		download_bottone_test.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent arg0) {
				JOptionPane.showMessageDialog(Interfaccia.frame,
					  ...
					  + "Italiano: \nLa domanda, l'unica della quale avrei voluto la risposta: � la ragazza giusta? E la risposta non � 42\n");
			}
		});
	}
				...
				if (filechooser.showOpenDialog(Interfaccia.frame) == 0) {
					...
					if(Settings.isWindows()){
						if(f.getName().compareToIgnoreCase("utorrent.exe")!=0){
							JOptionPane.showMessageDialog(frame, "L'unico client utilizzabile � uTorrent");
							return;
						}
					}
						...
						class CercaThread extends Thread{
							public void run(){
								...
								if(down_en){
									...
									sottotitoli_textarea_log.append(puntata.toString()+" � disponibile"+"\n");
								}
								else
					...
					public void actionPerformed(ActionEvent arg0) {
						class Thread_i extends Thread{
							public void run(){
								...
								if(res){
									sottotitoli_textarea_log.append(puntata.toString()+" � stato scaricato"+"\n");
									bot_rimuovi.doClick();
								}
							}
						}
					...
					public void actionPerformed(ActionEvent e) {
						...
						if(!OperazioniFile.subExistsFromPartialFilename(Settings.getDirectoryDownload()+torrent.getNomeSerieFolder(), nome_no_ext)){
							if(!GestioneSerieTV.getSubManager().scaricaSottotitolo(torrent)){
								JOptionPane.showMessageDialog(frame, "Non � associato alcun sottotitolo");
							}
						}
	...
	//TODO modificare per pi� gestori, come subsfactory
	...
	private static void creaTabAbout(){
		...
		about_label_donazione.addMouseListener(new MouseListener() {
			public void mouseReleased(MouseEvent arg0) {}
			//TODO modificare quando sar� presente browser per linux e mac
			public void mousePressed(MouseEvent arg0) {
				if(Settings.isWindows())
	...
	public static void ShowFrameOpzioni() {
		class FrameOpzioni {
			...
			private void create_view5(){
				...
				itasa_pass.setToolTipText("La password non verr� mostrata se gi� salvata precedentemente");
			...
			public void create_view7(){
				...
				JLabel testo=new JLabel("  La configurazione � terminata.  ");
