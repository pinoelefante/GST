...
public class Karmorra {
	...
	private void logga(){
		...
		try {
			...
			HtmlPage page_login = (HtmlPage) webClient.getPage("http://showrss.karmorra.info/?cs=login");
			HtmlForm form = page_login.getForms().get(0);
			
			HtmlSubmitInput button_submit = (HtmlSubmitInput) form.getInputByValue("Log in");
			HtmlTextInput textField_username = (HtmlTextInput) form.getInputByName("username");
			HtmlPasswordInput textField_password = (HtmlPasswordInput) form.getInputByName("password");
					
			textField_username.setValueAttribute("gst_username");//TODO inserire qui username
			textField_password.setValueAttribute("gestioneserietvpass");//TODO inserire qui password
			button_submit.click();
